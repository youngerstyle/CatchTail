...
`session_id` 来自 Codex hook payload。如果 payload 没有 session id，CLI 和 hook 必须报错停止，不能回退到 `default`。
...
`serve` 是每个 Codex session 独立启动的本地 sidecar。sidecar 在对应 session 的 milestone 变成 `completed` 时自动退出；如果用户 archive 了 Codex 线程，Codex 当前不会给插件发送 archive hook，因此 sidecar 依赖无活动 TTL 自动退出，默认 30 分钟，可通过 `CATCHTAIL_SIDECAR_IDLE_MS` 调整。
