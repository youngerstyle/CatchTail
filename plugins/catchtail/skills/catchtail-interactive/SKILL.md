...
- 首次启动时，在当前目标项目作为工作目录后台启动 hook 注入上下文里给出的完整 `node .../catchtail.js --session <id> serve 0` 命令，为当前 Codex session 启动独立 sidecar。保持该进程运行，捕获输出里的 `CatchTail Console: http://127.0.0.1:<port>` 和 `CatchTail Session: <session>` 后继续当前回合；不要假设固定端口，不要以前台常驻命令阻塞后续流程。没有 `--session <id>` 时 CLI 必须报错并停止，绝不能回退到 `default`。
- 启动控制台后，触发或发送“启动交互式工作流”，让 UserPromptSubmit hook 打开 interactive mode。
- 运行状态按 Codex hook 的 session_id 隔离；手动 CLI 必须显式传入 `--session <id>`，不能从环境猜测 session，也不能回退到 `default`。
