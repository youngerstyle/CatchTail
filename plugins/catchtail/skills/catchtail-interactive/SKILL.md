...
- 首次启动时，在当前目标项目作为工作目录后台启动 CatchTail CLI 的 `serve 0`，为当前 Codex session 启动独立 sidecar。保持该进程运行，捕获输出里的 `CatchTail Console: http://127.0.0.1:<port>` 和 `CatchTail Session: <session>` 后继续当前回合；不要假设固定端口，不要以前台常驻命令阻塞后续流程。CLI 会优先使用 `--session`，未显式传入时会读取 Codex 环境里的 `CODEX_SESSION_ID` / `CODEX_THREAD_ID`；如果这些都不存在，才会落到 `default`。
- 启动控制台后，触发或发送“启动交互式工作流”，让 UserPromptSubmit hook 打开 interactive mode。
- 运行状态按 Codex hook 的 session_id 隔离；手动 CLI 在 Codex 环境里默认使用当前 Codex session/thread，脱离 Codex 环境时才使用 default session。
