...
协议：
- 运行状态按 Codex hook 的 session_id 隔离；手动 CLI 必须显式传入 `--session <id>`，不能回退到 default session。
- queue.json 只保存当前未领取的队列消息。
- session.jsonl 保存追加式完整历史。
- 处理用户输入时，先运行 `node {{CLI_PATH}} --session <id> claim` 领取一条消息；处理后运行 `node {{CLI_PATH}} --session <id> complete <id> <简短处理结果>`。
...
- complete 后如果 milestone 仍未 completed，立即运行 `node {{CLI_PATH}} --session <id> wait`，保持当前回合继续等待。
